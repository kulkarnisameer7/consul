...
- `Permissions` `(array<IntentionPermission>)` - The list of all [additional L7
  ...
  Permissions are interpreted in the order represented in the list. For any
  given request the first permission to match in the list is terminal and stops
  further evaluation. As with L4 intentions, traffic that fails to match any of
  the provided permissions in this intention will be subject to the default
...
- `Type` `(string: <required>)` - The type for the `Name` value. This can be
  only `"consul"` today to represent a Consul service.
